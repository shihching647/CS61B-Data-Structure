package laboratory.lab13;/* UDGraph.java */

import homework.hw8.list.LinkedQueue;
import homework.hw8.list.Queue;
import homework.hw8.list.QueueEmptyException;
...
public class UDGraph
{
  ...
  public void BFSByLevel(UDGraph g, int startVertex, int level) throws QueueEmptyException {
    //init
    Queue queue = new LinkedQueue();
    queue.enqueue(startVertex);
    int curLevel = 0;
    int[] verticeCounts = new int[getNumVertices()]; //用來記錄每個level在queue裡還有多少個vertex

    //BFS by level
    while (curLevel < level && !queue.isEmpty()) {
      int start = (int) queue.dequeue();
      for (int i = 0; i < getNumVertices(); i++) {
        if (this.hasEdge(start, i)) {
          queue.enqueue(i);
          verticeCounts[curLevel + 1]++;
        }
      }
      if (--verticeCounts[curLevel] <= 0) //如果counts[curLevel] <= 0 -> 代表在queue裡,此curLevel的vertex已經沒了
        curLevel += 1;                    //進入下一個level
    }

    //將startVertex, 此level可達到的vertex紀錄在g裡面
    while (!queue.isEmpty()) {
      int dest = (int) queue.dequeue();
      g.addEdge(startVertex, dest);
    }
  }
  /**
   *  Returns a new UDGraph with the same vertices as "this" UDGraph.
