...
public class BinaryTree implements Dictionary {
  ...
  public Entry remove(Object key) {
    ...
    if (node == null) return null;
      ...
      if (node.leftChild == null && node.rightChild == null) {      //1.node沒有child
        if (node == root) {
          root = null;
        } else {
      ...
      } else if (node.leftChild == null || node.rightChild == null) { //2.node有一個child
        BinaryTreeNode childNode = node.leftChild != null ? node.leftChild : node.rightChild;
        if (node == root) {
          root = childNode;
        } else {
          if (isLeftChild(node)) node.parent.leftChild = childNode;
          else node.parent.rightChild = childNode;
          childNode.parent = node.parent;
        }
      } else {                                                        //3. node有兩個child
        BinaryTreeNode replaceNode = findSmallest(node.rightChild); //找到right child最小的那個元素
        node.entry = replaceNode.entry;
        //replaceNode.leftChild一定為null
        if (replaceNode.rightChild != null) //如果right child不為null, rightChild.parent指回replaceNode.parent
          replaceNode.rightChild.parent =replaceNode.parent;
        if (isLeftChild(replaceNode)) replaceNode.parent.leftChild = replaceNode.rightChild;
        else replaceNode.parent.rightChild = replaceNode.rightChild;
      }
    }
    size--;
    return node.entry;
  ...
  private boolean isLeftChild(BinaryTreeNode node) {
    ...
    else return node.parent.leftChild == node;
  }
  ...
  public static void main(String[] args) {
    ...
    //額外測試: 移除所有元素
    tree.testRemove(8, "(2Z)7S(10B)");
    System.out.println("Size:  " + tree.size);
    tree.testRemove(10, "(2Z)7S");
    System.out.println("Size:  " + tree.size);
    tree.testRemove(2, "7S");
    System.out.println("Size:  " + tree.size);
    tree.testRemove(7, "");
    System.out.println("Size:  " + tree.size);
//    tree.testRemove(7, "(2Z)8X(10B)");
//    System.out.println("Size:  " + tree.size);
//    tree.testRemove(8, "(2Z)10B");
//    System.out.println("Size:  " + tree.size);
//    tree.testRemove(10, "2Z");
//    System.out.println("Size:  " + tree.size);
//    tree.testRemove(2, "");
//    System.out.println("Size:  " + tree.size);
  }
